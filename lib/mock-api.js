...
function toCandidate(row) {
  if (!row) return null;
  const tokenCreatedAt = row.token_created_at instanceof Date ? row.token_created_at.toISOString() : row.token_created_at;
  const tokenExpiresAt = row.token_expires_at instanceof Date ? row.token_expires_at.toISOString() : row.token_expires_at;
  return {
    id: row.id,
    name: row.name,
    email: row.email,
    ...
    atsExplanation: row.ats_explanation,
    matchedRoleId: row.matched_role_id,
 ...
 * Validates token via candidates table (token must exist and not be expired).
 * @param {string} token
 * @param {object} data
...
export async function submitPreScreen(token, data) {
  if (!sql) return { data: null, error: "Database not configured." };
  try {
    const { getCandidateByInterviewToken } = await import("@/lib/services/candidate.service");
...
/**
 * Get job descriptions with openings > 0.
 * @returns {Promise<{ data: object[]; error: string | null }>}
 */
export async function getActiveJobDescriptions() {
  if (!sql) return { data: [], error: "Database not configured." };
  try {
 ...
 * @param {string} [payload.skills]
 * @param {number} [payload.experienceYears]
 * @param {string} [payload.education]
 * @param {string} [payload.atsExplanation]
 * @param {string} [payload.matchedRoleId]
 * @param {number} [payload.matchPercentage]
 * @param {string} [payload.matchReasoning]
 * @returns {Promise<{ data: object | null; error: string | null }>}
 */
export async function createCandidateMock(payload) {
  ...
  try {
    ...
    await sql`
      INSERT INTO candidates (id, name, email, phone, position, ats_score, interview_score, status, interview_date, token, skills, experience_years, education, ats_explanation, matched_role_id, match_percentage, match_reasoning)
      VALUES (${id}, ${name.trim()}, ${email.trim()}, ${phone?.trim() ?? null}, ${position?.trim() ?? "Unassigned"}, ${atsVal}, ${null}, ${"pending"}, ${null}, ${null}, ${skills?.trim() ?? null}, ${typeof experienceYears === "number" ? experienceYears : null}, ${education?.trim() ?? null}, ${atsExplanation?.trim() ?? null}, ${matchedRoleId?.trim() ?? null}, ${typeof matchPercentage === "number" ? matchPercentage : null}, ${matchReasoning?.trim() ?? null})
    `;
    const item = {
      id,
      name: name.trim(),
